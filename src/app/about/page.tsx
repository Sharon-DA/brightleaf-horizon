...
const journeySteps = [
  { date: "October 2, 2025", title: "Looking Ahead", content: "The world keeps evolving — and so do we. At BrightLeaf Horizon, innovation isn’t just about technology, but about human connection." },
  { date: "October 1, 2025", title: "Milestones Achieved", content: "From shaping digital identities for architectural visionaries to advancing sustainable energy narratives." },
  { date: "September 22, 2025", title: "Founding Principles", content: "Founded in 2019, BrightLeaf Horizon was born from a simple but powerful idea: to empower brands with strategy and soul." }
];
    ...
    <div className="bg-white">
      ...
      <section className="pt-32 pb-20 px-6 bg-slate-50">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-4xl md:text-6xl font-bold text-slate-900 mb-6">Empowering Sustainable Growth</h1>
          ...
          <div className="flex justify-center gap-4">
             <Link href="#journey" className="bg-amber-600 text-white px-8 py-3 rounded-lg font-medium hover:bg-amber-700 transition">Learn More</Link>
          </div>
        </div>
      </section>
        ...
        <div className="grid md:grid-cols-3 gap-12">
          {experts.map((expert, i) => (
            <div key={i} className="flex flex-col items-center text-center group">
              <div className="w-64 h-64 mb-6 rounded-full overflow-hidden border-4 border-white shadow-lg transition-transform group-hover:scale-105">
                <img src={expert.image} alt={expert.name} className="w-full h-full object-cover" />
              </div>
              <h3 className="text-2xl font-bold text-slate-900 mb-2">{expert.name}</h3>
              <p className="text-amber-600 italic mb-4 font-medium px-4">"{expert.quote}"</p>
      ...
      <section className="py-24 px-6 bg-slate-900 text-white">
        <div className="max-w-7xl mx-auto">
          <div className="mb-16">
            ...
            <p className="text-amber-400">For us, sustainability isn’t a checkbox — it’s how we think, create, and grow.</p>
          </div>
          ...
          {journeySteps.map((step, i) => (
            <div key={i} className="flex flex-col md:flex-row gap-4 md:gap-12 border-l-2 border-amber-500 pl-8 relative">
              ...
              <div className="min-w-[150px]">
                <p className="text-amber-600 font-bold text-sm uppercase tracking-wider">{step.date}</p>
              </div>
              <div>
